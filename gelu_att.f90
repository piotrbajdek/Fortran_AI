...
program gelu_att
  ...
  do i=1, iterations
      ...
      k = net_size + 1
      do j=k, seq_len
          hidden_state(j) = gelu(dot_product(w1, sequence(j-net_size:j-1)) + b1) + &
                            gelu(dot_product(w2, sequence(j-net_size:j-1)) + b2) + &
                            gelu(dot_product(w3, sequence(j-net_size:j-1)) + b3)
  ...
  y_pred = gelu(dot_product(w1, sequence(seq_len-net_size:seq_len-1)) + b1) + &
           gelu(dot_product(w2, sequence(seq_len-net_size:seq_len-1)) + b2) + &
           gelu(dot_product(w3, sequence(seq_len-net_size:seq_len-1)) + b3)
...
contains
  ...
  real(qp) function gelu(x)
    real(qp), intent(in) :: x
    gelu = 0.5_qp * x * (1.0_qp + tanh(0.797885*x + 0.0356774*x**3))
  end function gelu

end program gelu_att
