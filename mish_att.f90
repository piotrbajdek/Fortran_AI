...
program mish_att
  ...
  do i=1, iterations
      ...
      k = net_size + 1
      do j=k, seq_len
          hidden_state(j) = mish(dot_product(w1, sequence(j-net_size:j-1)) + b1) + &
                            mish(dot_product(w2, sequence(j-net_size:j-1)) + b2) + &
                            mish(dot_product(w3, sequence(j-net_size:j-1)) + b3)
  ...
  y_pred = mish(dot_product(w1, sequence(seq_len-net_size:seq_len-1)) + b1) + &
           mish(dot_product(w2, sequence(seq_len-net_size:seq_len-1)) + b2) + &
           mish(dot_product(w3, sequence(seq_len-net_size:seq_len-1)) + b3)
