...
program rnn_mh_sc_att_3
  ...
  ! Scale factor for attention (sqrt of the dimension of the key vectors)
  scale_factor = sqrt(3.0_dp) / sqrt(real(num_heads))
  ...
  do i=1, iterations
      ...
      do head = 1, num_heads
          ...
          attention_weights(:, :, head) = attention_weights(:, :, head) * scale_factor
      end do
